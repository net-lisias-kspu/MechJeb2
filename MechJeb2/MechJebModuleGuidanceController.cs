...
namespace MuMech
{
    ...
    public class MechJebModuleGuidanceController : ComputerModule
    {
        ...
        private bool WillDoRCSButNotYet()
        {
            ...
            return hasRCS && Status != PSGStatus.TERMINAL_RCS && Vessel.currentStage == Solution.TerminalStage();
        }
        ...
        private void HandleTerminal()
        {
            ...
            if (Vessel.currentStage < Solution.TerminalStage())
            {
                Done();
                return;
            }
            ...
            if (Solution.OptimizeStage() < 0 && Vessel.currentStage <= Solution.TerminalStage() && Solution.Tgo(VesselState.time) <= 0 &&
            ...
            {
                if (Vessel.currentStage == Solution.OptimizeStage())
                    return;
            ...
            if (Vessel.currentStage != Solution.OptimizeStage() && Solution.Tgo(VesselState.time) > 10)
                return;
            ...
            if (Status == PSGStatus.TERMINAL && VesselState.thrustCurrent == 0 && Vessel.currentStage < Solution.TerminalStage())
            {
        ...
        private void TerminalDone()
        {
            ...
            if (Solution.TerminalStage() != Vessel.currentStage && Solution.OptimizeStage() == Vessel.currentStage)
            {
                ThrustOff();
