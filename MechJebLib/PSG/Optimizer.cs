...
namespace MechJebLib.PSG
{
    public partial class Optimizer : IDisposable
    {
        ...
        private      int    K => 2 * N - 1;
        public const double MINDT = 1.4901161193847656e-8;
        ...
        public double Epsx             { get; set; } = 1e-4;
        ...
        private void ConstraintFunction(double[] x, double[] f, alglib.sparsematrix j, object? o)
        {
            ...
            for (int p = 0; p < _numPhases; p++)
            {
                ...
                for (int n = 0; n < N - 1; n += 1)
                {
                    ...
                    // dr_x/dt = v_x
                    //DebugPrint($"{ci}: dr_x dynamic constraint");
                    ...
                    // dr_y/dt = v_y
                    //DebugPrint($"{ci}: dr_z dynamic constraint");
                    ...
                    //DebugPrint($"{ci}: dv_x dynamic constraint");
                    ...
                    //DebugPrint($"{ci}: dv_y dynamic constraint");
                    ...
                    //DebugPrint($"{ci}: dv_z dynamic constraint");
                    ...
                    // dm/dt = -mdot
                    //DebugPrint($"{ci}: mass dynamic constraint");
                ...
                // complementary constraints on the burn times
                ...
                else
                {
                    ...
                    f[ci++] = (x[nextBurnPhase.Tf] - x[nextBurnPhase.Ti] - MINDT) * (_phases[p].bt - (x[thisPhase.Tf] - x[thisPhase.Ti]));
                    alglib.sparseappendemptyrow(j);
                    alglib.sparseappendelement(j, thisPhase.Ti, x[nextBurnPhase.Tf] - x[nextBurnPhase.Ti] - MINDT);
                    alglib.sparseappendelement(j, thisPhase.Tf, x[nextBurnPhase.Ti] - x[nextBurnPhase.Tf] + MINDT);
                    alglib.sparseappendelement(j, nextBurnPhase.Ti, -(_phases[p].bt - (x[thisPhase.Tf] - x[thisPhase.Ti])));
                    alglib.sparseappendelement(j, nextBurnPhase.Tf, _phases[p].bt - (x[thisPhase.Tf] - x[thisPhase.Ti]));
                }
        ...
        private (double t0, double oldt0) TranscribePhasesFromOldSolution(int phaseStart, int phaseLimit, double t0, Solution oldSolution, double oldt0, double oldtf)
        {
            ...
            for (int p = phaseStart; p < phaseLimit; p++)
            {
                ...
                oldtbt = oldtf - oldt0;
                double bt    = _phases[p].Coast ? oldtbt : Max(Min(phase.bt, oldtbt / frac), MINDT);
        ...
        public void TranscribePreviousBootSolution(Solution oldSolution)
        {
            ...
            for (int p = 0; p < _phases.Count; p++)
            {
                ...
                double oldtf = t0 + oldbt;
                double oldh  = (oldtf - t0) / (K - 1);
        ...
        private Solution UnSafeRun()
        {
            ...
            double[] nl   = new double[VariableLayout.NumConstraintsForPhases(N, K, _numPhases, _problem)];
            double[] nu   = new double[VariableLayout.NumConstraintsForPhases(N, K, _numPhases, _problem)];
            ...
            for (int p = 0; p < _numPhases; p++)
            {
                ...
                {
                    DebugPrint($"BOUNDS: optimize time constraint index: {78 + 124 * p}");
                    nl[78 + 124 * p] = MINDT;
                    nu[78 + 124 * p] = double.PositiveInfinity;
                }
                else
                {
                    DebugPrint($"BOUNDS: non-optimize time constraint index: {78 + 124 * p}");
                    nl[78 + 124 * p] = MINDT;
                    nu[78 + 124 * p] = _phases[p].bt;
                }

                // complementary constraint on burn time
                nl[79 + 124 * p] = double.NegativeInfinity;
                nu[79 + 124 * p] = 1e-6;
            }
            ...
            if (ogrep.badgradsuspected)
                if (!DoubleMatrixSparsityValidation(ogrep.badgraduser, ogrep.badgradnum, 1e-3))
                    throw new Exception(
                        $"badgradsuspected: {ogrep.badgradfidx},{ogrep.badgradvidx} {ogrep.badgraduser[ogrep.badgradfidx, ogrep.badgradvidx]} != {ogrep.badgradnum[ogrep.badgradfidx, ogrep.badgradvidx]}\nuser:\n{DoubleMatrixString(ogrep.badgraduser)}\nnumerical:\n{DoubleMatrixString(ogrep.badgradnum)}\nsparsity check:\n{DoubleMatrixSparsityCheck(ogrep.badgraduser, ogrep.badgradnum, 1e-3)}");
        ...
        private Solution GenerateSolution(double[] x)
        {
            ...
            for (int p = 0; p < _numPhases; p++)
            {
                ...
                if (_phases[p].AllowShutdown && tf - ti > 10 * MINDT)
                    optimizedShutdownIndex = p;

                // XXX: this is even jankier.
                _phases[optimizedShutdownIndex].PreciseShutdown = false;
